...
Forge is a conversation-first Roblox Studio creator harness with a separate
registered-benchmark system. In an ordinary project conversation, the creator
opens a place, sends a hash-bound turn, reviews or refines a visible plan and
verification charter, approves an exact typed change set, runs the checks in
Studio, and accepts or rolls back the result. Each change set has exactly one
declared persistent writer: the open Studio document by default, or an
explicitly opted-in Rojo source tree.
...
## Creator conversation in Studio
...
After replacing the connector, restart Studio so its loaded plugin and the
rebuilt service agree. Link/Fork failures now show their exact retained error
and technical evidence. **Retry linking** appears only after an exact no-effect
rejection; an unknown outcome instead requires fresh pairing/recovery. Neither
the dashboard nor the service retries linking automatically.

Open any platform-valid place in Studio—including an existing `.rbxlx` with no Forge metadata—then open the one-time dashboard URL, allow local HTTP and script injection, and wait for the connector to pair. For an unpublished local place, use the hash-bound **Link project** action before starting conversation work; the dedicated identity receipt writes `_forgeProjectId` through its own ChangeHistory recording and direct readback. **Fork project** is the explicit Save As boundary. If a linked local place later becomes published, choose **Continue this conversation** or **Start new project conversation**; Forge never infers continuity from a filename.

Forge first collects a complete sharded, content-addressed project index and the current Script Editor buffers. Duplicate names and unsupported classes remain indexable because mutation authority uses opaque connector-epoch identities rather than display paths. The planner can search and page source, inspect symbols/references, and traverse static require dependencies before proposing a plan; the exact consulted ranges and dependency closure are immutable evidence. Submit the prompt and follow only the dashboard's current hash-bound actions. Plan approval and exact change approval remain explicit decisions. After matched provisional Apply, Forge silently arms the next ordinary Play session; press Play whenever ready, perform the approved interaction, then press Stop. Complete evidence advances even when it proves a mismatch. Technical incompleteness shows **Retry Play Verification** / **Cancel Changes** and never silently re-arms. After acceptance, the committed result exists in Studio; export it explicitly with **File → Save to File** and choose `.rbxlx`. Forge makes no filesystem-output claim before that creator action.

Creator jobs run only while `creator serve` remains running. Closing the browser does not stop a durably admitted foreground job; stopping the service does. Every planner, builder, and repair AgentRun has an append-only `AgentExecutionJournal`. On restart, a job with no journal head exposes **Resume work** with a fresh AgentRun/journal. A `request_intent` without `response_received`, or a `tool_execution_intent` without its matching `tool_completed`, remains `outcome_unknown`; **Retry work** requires explicit creator authority and reserves a fresh AgentRun/journal rather than retrying automatically. In contrast, a fully persisted provider response with every completed tool record and no pending tool-execution intent exposes **Resume work**: the creator-authorized resume consumes that exact boundary in the same reserved AgentRun/journal without re-dispatching the persisted response. Service downtime does not consume the checkpointed active-execution duration budget, while the final runtime interval still records the complete wall-clock gap. Forge never automatically retries or dispatches provider, Studio, Apply, commit, cancellation, rollback, or Play work.
...
The dashboard's compact Roblox API ledger shows the source pin, catalog/coverage/manifest identities, connector attestation health, signatures, security, capability tags, official YAML provenance, inheritance, codec, disposition, reason, and seven-stage proof route. It is an accountability view, not an alternate mutation menu; the public `CreatorControlView` remains the only workflow-legality contract. The lower transaction coordinator separately uses `CreatorTransactionControlView` and its bounded ordered actions; their intent labels do not create a second public control surface.
...
forge creator replay-verification|replay-mutation <session-id>
...
`creator act --model` is valid only for a current **Change the plan** action.
When it is omitted, that action uses the sealed registry default. It cannot
alter the model already bound to an approved plan's builder or repair run, and
Forge never silently substitutes a model or provider.
