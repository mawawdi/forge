...
- Forge compiles the complete dependency-ordered runtime proof program before place mutation, requires human-triggered position observations to span at least 15 seconds within the 20-second runtime budget, and persists the exact post-revision-bound plan before offering the check action;
