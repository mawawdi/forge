...
- Forge compiles the complete dependency-ordered runtime proof program before place mutation, reserves one bounded maximum creator-observation window inside the generated five-minute Studio ceiling, and persists plus reads back the exact post-revision-bound passive arm before the next ordinary Studio Play session; human Play authority has no transport timeout, and the machine deadline begins only after the exact Play Server start message;
- the creator presses Studio Play and Stop normally; a distinct Play Server connector resumes only the exact durable arm, installs diagnostics and direct manifest dispatch, records bounded facts in memory, samples concurrent series through Heartbeat, persists the Stop-sealed terminal receipt before transport, and Forge never starts or stops that creator-driven session;
...
- the Edit connector may turn a stopped passive arm into an inert replay-only tombstone only after an exact backend finalization request; incomplete runtime evidence moves to a visible retry/cancel authority state, and only an exact creator retry may replace that same-transaction tombstone with one new arm; terminal cleanup waits for durable mutation-finalization acknowledgement, and ambiguous runtime state never triggers automatic retry, cancellation, repair, commit, or an invented verdict;
...
- mismatched or incomplete mutation evidence cancels the exact recording and requires post-cancel state evidence; runtime-evidence incompleteness follows the non-destructive explicit-retry rule above, and missing evidence is never converted into a fabricated mismatch;
