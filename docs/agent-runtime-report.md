...
- Tool schemas and descriptions are versioned harness behavior and participate in configuration identity.
...
- system prompt;
- versioned tool schemas and descriptions;
- workspace/capability policy;
- initial-orientation policy and content hash;
- resolved requirement-view hash;
- budget policy;
- native runtime identity/version;
- model transport, client version, and exact model ID;
- pinned AI SDK/provider-adapter versions, provider routing, reasoning, retry, timeout, output, one-step, telemetry, and continuation policies.
