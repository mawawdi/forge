...
The OpenRouter adapter loads `OPENROUTER_API_KEY` only at the CLI/model boundary. The key is excluded from semantic identity, request hashes, `HarnessConfiguration`, `AgentRun`, and traces. A registered experiment requires an explicit `--model`; Creator Conversation instead binds the registry-selected model to the exact turn or plan revision. In both paths, provider/model identity is execution configuration rather than creator or evaluator authority.
