...
export interface CreatorOrientationContent {
  ...
  studioAuthoring: {
    ...
    allowedOperations: Array<"create" | "update" | "move" | "delete" | "write_source">;
    ...
    machineChecks: Array<"instance_exists" | "position_series" | "playtest_diagnostics" | "subtree_unchanged">;
    checkScopes: {
...
export function compileAgentOrientation(input: { semanticMap: ProjectSemanticMap; projectSnapshotHash: string; requirementView: RequirementView; sourceRoots: readonly string[] }): AgentOrientation {
  const visibleRequirements = input.requirementView.decisions
    ...
    .map((decision) => ({ id: decision.requirement.id, statement: decision.requirement.statement, enforcement: decision.requirement.enforcement, verificationModes: [...decision.requirement.verificationModes] }))
  ...
  const content: ExperimentOrientationContent = {
    ...
    instances: input.semanticMap.instances.map((instance) => ({ id: instance.id, path: instance.path, className: instance.className, ...(instance.position ? { position: { ...instance.position } } : {}) })).sort((left, right) => left.path.localeCompare(right.path) || left.id.localeCompare(right.id)),
    visibleRequirements
  };
...
export function compileCreatorOrientation(input: { observation: StudioSnapshotObservation; revisionHash: string; projectId: string; ownership: ReadonlyMap<string, "studio" | "external_rojo">; allowedClasses: readonly string[]; resolvableClasses: readonly string[] }): AgentOrientation {
  const content: CreatorOrientationContent = {
    ...
    studioAuthoring: {
      ...
      allowedOperations: ["create", "update", "move", "delete", "write_source"],
      createInitialization: { scriptClasses: "inline_source_required", nonScriptClasses: "initial_properties" },
      writeSourceTargets: "initial_snapshot_scripts_only",
      createAndMoveParents: "must_exist_in_initial_snapshot_and_be_studio_writable",
      plannedInstancesMayParentPlannedInstances: false,
      machineChecks: ["instance_exists", "playtest_diagnostics", "position_series", "subtree_unchanged"],
      checkScopes: { instanceExists: "allowlisted_studio_roots", positionSeries: "Workspace_BasePart_only", subtreeUnchanged: "initial_snapshot_allowlisted_studio_roots" },
      machineCheckStatements: "forge_generated_from_typed_fields",
      arbitraryCodeExecution: false,
      genericPropertyAccess: false
    }
...
function orientation(mode: OrientationMode, projectSnapshotHash: string, content: AgentOrientation["content"], requirementViewId?: string): AgentOrientation {
  const contentHashValue = contentHash(stableJson(content));
  const identity = contentHash(stableJson({ policy: "mode_scoped_project_capabilities", mode, ...(requirementViewId ? { requirementViewId } : {}), projectSnapshotHash, contentHash: contentHashValue }));
  return { kind: "AgentOrientation", id: `agent_orientation_${identity.slice(0, 24)}`, policy: "mode_scoped_project_capabilities", mode, ...(requirementViewId ? { requirementViewId } : {}), projectSnapshotHash, content, contentHash: contentHashValue };
}
...
function isCanonicalRelativeRoot(value: string): boolean {
  return value.length > 0 && !value.includes("\0") && !value.includes("\\") && !value.includes(":") && !value.startsWith("/") && value.split("/").every((segment) => segment.length > 0 && segment !== "." && segment !== "..");
}
