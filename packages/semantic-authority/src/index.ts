...
export type RequirementVerificationMode = "schema" | "static" | "preflight" | "studio" | "evaluator" | "human";
...
export type RequirementViewDecision =
  | (RequirementViewDecisionBase & { visible: true; withheld: false; requirementId: string; requirement: Requirement })
  | (Omit<RequirementViewDecisionBase, "requirementId" | "visible" | "withheld"> & { visible: false; withheld: true });
...
const SOURCES: readonly RequirementSource[] = ["creator", "project_observation", "platform_policy", "agent_plan", "evaluator", "benchmark_oracle"];
const AUTHORITIES: readonly RequirementAuthority[] = ["fact", "policy", "hypothesis", "evaluation_only"];
const VISIBILITIES: readonly RequirementVisibility[] = ["builder_visible", "evaluator_only", "internal"];
const ENFORCEMENTS: readonly RequirementEnforcement[] = ["informational", "advisory", "blocking"];
const VERIFICATION_MODES: readonly RequirementVerificationMode[] = ["schema", "static", "preflight", "studio", "evaluator", "human"];
...
export function resolveRequirementView(requirementSet: RequirementSet, scope: RequirementScope): RequirementView {
  assertRequirementSet(requirementSet);
  assertRequirementScope(scope);
  const decisions = requirementSet.requirements.map((requirement) => resolveRequirement(requirement, scope));
...
export function assertRequirement(value: unknown): asserts value is Requirement {
  ...
  if (!Array.isArray(value.verificationModes) || value.verificationModes.length === 0 || !value.verificationModes.every((mode) => includes(VERIFICATION_MODES, mode)) || !isCanonicalUniqueStrings(value.verificationModes)) {
    throw new Error("Invalid Requirement: verificationModes must be non-empty, unique, and canonical");
  }
  if (!Array.isArray(value.evidence) || value.evidence.length === 0) throw new Error("Invalid Requirement: evidence is required");
  ...
  if (!isCanonicalUniqueBy(evidence, (item) => item.id)) throw new Error("Invalid Requirement: evidence IDs must be unique and canonical");
  if (evidence.some((item) => item.kind !== EVIDENCE_KIND_BY_SOURCE[value.source as RequirementSource])) throw new Error("Invalid Requirement: evidence does not match source");
  ...
  if (source === "benchmark_oracle" && (authority !== "evaluation_only" || visibility !== "evaluator_only")) throw new Error("Invalid Requirement: benchmark oracles must be evaluation-only and evaluator-only");
  if (source === "project_observation" && authority !== "fact") throw new Error("Invalid Requirement: project observations must be factual");
  if (source === "agent_plan" && authority !== "hypothesis") throw new Error("Invalid Requirement: agent plans cannot declare policy authority");
  if (authority === "hypothesis" && enforcement === "blocking") throw new Error("Invalid Requirement: hypotheses cannot be blocking");
  if (source === "platform_policy") {
    if (authority !== "policy" && authority !== "hypothesis") throw new Error("Invalid Requirement: platform policy must be policy or hypothesis authority");
    if (enforcement === "blocking") {
      if (authority !== "policy" || !evidence.every((item) => item.kind === "policy_reference" && item.policyId !== value.id)) throw new Error("Invalid Requirement: blocking platform policy requires independent policy provenance");
    }
  }
}
...
export function assertRequirementSet(value: unknown): asserts value is RequirementSet {
  ...
  if (!isCanonicalUniqueBy(requirements, (item) => item.id)) throw new Error("Invalid RequirementSet: requirement IDs must be unique and canonical");
  if (value.id !== requirementSetId(requirements)) throw new Error("Invalid RequirementSet: ID does not match canonical content");
}
...
export function assertAcceptanceSpec(value: unknown): asserts value is AcceptanceSpec {
  ...
  if (requirementIds.length === 0) throw new Error("Invalid AcceptanceSpec: at least one requirement reference is required");
  ...
  if (value.id !== expectedId) throw new Error("Invalid AcceptanceSpec: ID does not match canonical references");
}

export function assertAcceptanceSpecReferences(spec: AcceptanceSpec, requirementSet: RequirementSet): void {
  assertAcceptanceSpec(spec);
  assertRequirementSet(requirementSet);
  if (spec.requirementSetId !== requirementSet.id) throw new Error("Invalid AcceptanceSpec references: requirement set does not match");
  ...
  if (missing.length > 0) throw new Error(`Invalid AcceptanceSpec references: unknown requirements ${missing.join(", ")}`);
}
...
export function assertIntegrationConstraint(value: unknown): asserts value is IntegrationConstraint {
  ...
  if (value.id !== expectedId) throw new Error("Invalid IntegrationConstraint: ID does not match canonical references");
}

export function assertIntegrationConstraintReferences(constraint: IntegrationConstraint, requirementSet: RequirementSet): void {
  assertIntegrationConstraint(constraint);
  assertRequirementSet(requirementSet);
  const requirement = requirementSet.requirements.find((candidate) => candidate.id === constraint.requirementId);
  if (!requirement || requirement.source !== "project_observation" || requirement.authority !== "fact") throw new Error("Invalid IntegrationConstraint references: requirement must be a project observation fact");
  const matchingEvidence = requirement.evidence.some((evidence) => evidence.kind === "project_observation" && evidence.projectId === constraint.projectId && evidence.projectSnapshotHash === constraint.projectSnapshotHash);
  if (!matchingEvidence) throw new Error("Invalid IntegrationConstraint references: observation evidence does not match project snapshot");
}
...
export function assertRequirementIdentityStable(previous: Requirement, next: Requirement): void {
  ...
  if (previous.id !== next.id) throw new Error("Requirement identity comparison requires the same requirement ID");
  if (previous.source !== next.source || previous.authority !== next.authority) throw new Error("Requirement source and authority are immutable for an existing ID");
}

export function assertRequirementSetIdentityStable(previous: RequirementSet, next: RequirementSet): void {
...
function canonicalRequirement(requirement: Requirement): Requirement {
  const canonical: Requirement = {
    ...
    evidence: requirement.evidence.map((item) => ({ ...item })).sort((left, right) => compareStrings(left.id, right.id))
  };
  assertRequirement(canonical);
  return canonical;
}

function resolveRequirement(requirement: Requirement, scope: RequirementScope): RequirementViewDecision {
  const reasons: string[] = [];
  const benchmarkInScope = requirement.source !== "benchmark_oracle" || (scope.environment === "benchmark" && scope.phase === "evaluate" && scope.audience !== "builder");
  const visibilityAllowed = scope.audience === "internal"
    ? true
  ...
  if (!benchmarkInScope) reasons.push(requirement.source === "benchmark_oracle" && scope.audience === "builder" ? "benchmark_oracle_hidden_from_builder" : "benchmark_oracle_out_of_scope");
  else if (visible) reasons.push("visible_to_audience");
  else reasons.push(requirement.visibility === "internal" ? "internal_requirement_withheld" : "evaluator_only_requirement_withheld");

  const evaluationInScope = requirement.authority !== "evaluation_only" || scope.phase === "evaluate";
  const hiddenPolicyCanEnforce = requirement.visibility === "internal" && requirement.source === "platform_policy" && requirement.authority === "policy";
  const audienceCanEnforce = visible || hiddenPolicyCanEnforce;
  const enforceable = benchmarkInScope && evaluationInScope && audienceCanEnforce && requirement.enforcement !== "informational";
...
function assertRequirementEvidence(value: unknown): asserts value is RequirementEvidence {
  if (!isRecord(value) || !isId(value.id) || !isNonEmptyString(value.kind)) throw new Error("Invalid Requirement evidence");
  switch (value.kind) {
    case "creator_request":
      if (!hasExactKeys(value, ["kind", "id", "intentId", "requestHash"]) || !isId(value.intentId) || !isHash(value.requestHash)) throw new Error("Invalid creator request evidence");
      return;
    ...
    case "evaluation_spec":
      if (!hasExactKeys(value, ["kind", "id", "evaluationId", "criterionId", "specificationHash"]) || !isId(value.evaluationId) || !isId(value.criterionId) || !isHash(value.specificationHash)) throw new Error("Invalid evaluation specification evidence");
      return;
    case "benchmark_fixture":
      if (!hasExactKeys(value, ["kind", "id", "benchmarkId", "oracleId", "fixtureHash"]) || !isId(value.benchmarkId) || !isId(value.oracleId) || !isHash(value.fixtureHash)) throw new Error("Invalid benchmark fixture evidence");
      return;
    default:
...
function assertRequirementScope(value: unknown): asserts value is RequirementScope {
  if (!isRecord(value) || !hasExactKeys(value, ["phase", "environment", "audience"]) || !["build", "evaluate"].includes(String(value.phase)) || !["production", "benchmark"].includes(String(value.environment)) || !["builder", "evaluator", "internal"].includes(String(value.audience))) throw new Error("Invalid RequirementScope");
}
...
function isCanonicalUniqueStrings(values: readonly unknown[]): values is string[] {
  return values.every((value) => typeof value === "string") && isCanonicalUniqueBy(values as string[], (value) => value);
}
