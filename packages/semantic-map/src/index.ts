...
import {
  ...
  type RelativePath,
  type RemoteFlowDeclaration,
} from "../../contracts/src/index.js";
import type { StudioProjectAttributeValue } from "../../studio-evidence/src/index.js";
...
export interface SemanticInstance {
  ...
  attributes: Record<string, StudioProjectAttributeValue>;
  tags: string[];
}
...
export class FilesystemProjectSourceAdapter implements ProjectSourceAdapter {
  load(input: { root: string; manifest: ForgeFixtureManifest }): Promise<ProjectSemanticMap> {
    return buildSemanticMap(input.root, input.manifest);
  }
  snapshot(map: ProjectSemanticMap): ProjectSnapshot {
    return createProjectSnapshot(map);
  }
}
...
export async function buildSemanticMap(
  ...
  for (const sourceRoot of [...manifest.luauRoots].sort())
    await collectLuauFiles(canonicalRoot, sourceRoot, files);
  files.sort((left, right) => left.path.localeCompare(right.path));
  const byPath = new Map(files.map((file) => [file.path, file]));
  const remoteFlows = manifest.remoteFlows.map((declaration) => ({
    declaration,
    client:
      byPath.get(normalize(declaration.clientScript)) ??
      missingSource(canonicalRoot, declaration.clientScript, "client"),
    server:
      byPath.get(normalize(declaration.serverScript)) ??
      missingSource(canonicalRoot, declaration.serverScript, "server"),
  }));
  const scripts = files.map((file) => ({
    id: stableId("script", file.path),
    path: file.path,
    executionContext: file.executionContext,
    sourceHash: contentHash(file.source),
    dependencies: findDependencies(file.source),
  }));
  const instances = buildInstances(files, manifest);
  const remotes = manifest.remoteFlows
    .map((flow) => ({
      id: flow.remote.stableId,
      path: flow.remote.path,
      name: flow.name,
      className: flow.remote.className,
      direction: flow.direction,
      clientScript: normalize(flow.clientScript),
      serverScript: normalize(flow.serverScript),
    }))
    .sort((left, right) => left.path.localeCompare(right.path));
  const dependencies = [
    ...scripts.flatMap((script) =>
      script.dependencies.map((dependency) => ({
        from: script.path,
        to: dependency,
    ...
    ...remotes.flatMap((remote) => [
      { from: remote.clientScript, to: remote.id, kind: "remote" as const },
      { from: remote.serverScript, to: remote.id, kind: "remote" as const },
    ]),
  ].sort((left, right) => `${left.from}|${left.to}`.localeCompare(`${right.from}|${right.to}`));
  const partial: Omit<ProjectSemanticMap, "hashes"> = {
    kind: "ProjectSemanticMap",
    projectId: stableId("project", manifest.name),
    root: canonicalRoot,
    files,
    scripts,
    instances,
...
export function createProjectSnapshot(map: ProjectSemanticMap): ProjectSnapshot {
  ...
  const projectSemanticHash = contentHash(
    stableJson({
      sourceHash: map.hashes.sourceHash,
      structureHash: map.hashes.structureHash,
      semanticHash: map.hashes.semanticHash,
    }),
  );
  return {
    kind: "ProjectSnapshot",
...
export function canonicalProjectSemanticMap(map: ProjectSemanticMap): Record<string, unknown> {
  return {
    ...
    files: map.files
      ...
      .sort(byPath),
    scripts: map.scripts
      .map((script) => ({ ...script, dependencies: [...script.dependencies].sort() }))
      .sort(byPath),
    instances: map.instances
      .map((instance) => ({
        ...instance,
        properties: sorted(instance.properties),
        attributes: sorted(instance.attributes),
        tags: [...instance.tags].sort(),
      }))
      .sort(byPath),
    remotes: [...map.remotes].sort(byPath),
    remoteFlows: map.remoteFlows
      .map((flow) => ({
        declaration: flow.declaration,
        clientHash: contentHash(flow.client.source),
        serverHash: contentHash(flow.server.source),
      }))
...
export function assertProjectSemanticMap(value: unknown): asserts value is ProjectSemanticMap {
  if (
    ...
    throw new Error("Invalid ProjectSemanticMap");
}
...
async function collectLuauFiles(
  ...
  for (const entry of entries.sort((left, right) => left.name.localeCompare(right.name))) {
    ...
    if (metadata.isSymbolicLink())
      throw new Error(
        `Source root contains a symbolic link: ${normalize(relative(root, absolutePath))}`,
      );
    if (metadata.isDirectory())
      await collectLuauFiles(root, normalize(relative(root, absolutePath)), output);
      ...
      output.push({
        ...
        executionContext: inferContext(path),
      });
    }
  }
}
...
function buildInstances(files: SourceFile[], manifest: ForgeFixtureManifest): SemanticInstance[] {
  const declared = (manifest.instances ?? []).map((instance) => ({
    id: stableId("instance", `${instance.path}|${instance.className}`),
    path: instance.path,
  ...
  for (const file of files) {
    ...
    if (!declared.some((instance) => instance.path === path))
      declared.push({
        id: stableId("instance", `${path}|${scriptClass(file)}`),
        path,
        className: scriptClass(file),
        properties: {},
        attributes: {},
        tags: [],
      });
...
function projectHashes(map: Omit<ProjectSemanticMap, "hashes">): ProjectSemanticMap["hashes"] {
  const sourceHash = contentHash(
    stableJson(map.files.map((file) => ({ path: file.path, source: file.source }))),
  );
  const structureHash = contentHash(stableJson({ instances: map.instances, remotes: map.remotes }));
  const semanticHash = contentHash(
    stableJson({
      scripts: map.scripts,
      remoteFlows: map.remoteFlows.map((flow) => flow.declaration),
      dependencies: map.dependencies,
    }),
  );
...
function missingSource(
  root: string,
  path: string,
  context: SourceFile["executionContext"],
): SourceFile {
  return {
    path: normalize(path),
    absolutePath: inside(root, path),
    source: "",
    executionContext: context,
  };
}
function findDependencies(source: string): string[] {
  return [...source.matchAll(/require\s*\(\s*["']([^"']+)["']\s*\)/g)]
    .map((match) => normalize(match[1]!))
    .sort();
}
function inferContext(path: string): SourceFile["executionContext"] {
  if (/\.server\.(?:lua|luau)$/.test(path) || path.includes("/server/")) return "server";
  if (/\.client\.(?:lua|luau)$/.test(path) || path.includes("/client/")) return "client";
  if (path.includes("/shared/") || path.includes("/ReplicatedStorage/")) return "shared";
  return "unknown";
}
function scriptClass(file: SourceFile): string {
  return file.executionContext === "server"
    ? "Script"
    : file.executionContext === "client"
      ? "LocalScript"
      : "ModuleScript";
}
function stableId(kind: string, value: string): string {
  return `${kind}_${contentHash(value).slice(0, 24)}`;
}
function normalize(value: string): string {
  return value.replaceAll("\\", "/").replace(/^\.\//, "");
}
function sorted<T>(values: Record<string, T>): Record<string, T> {
