...
export const STUDIO_RESOLVABLE_CLASSES = ["BasePart", "Folder", "LocalScript", "Model", "ModuleScript", "Part", "ProximityPrompt", "RemoteEvent", "RemoteFunction", "Script"] as const;
export type StudioResolvableClass = typeof STUDIO_RESOLVABLE_CLASSES[number];
/** Explicit Studio service roots that the fixed resolver may inspect. */
export const STUDIO_INSTANCE_RESOLUTION_ROOTS = ["Lighting", "ReplicatedFirst", "ReplicatedStorage", "ServerScriptService", "ServerStorage", "SoundService", "StarterGui", "StarterPack", "StarterPlayer", "Teams", "Workspace"] as const;
...
export interface StudioCapabilityPolicy {
  /** Applies only to the read-only instance.resolve capability. */
  allowedRoots: StudioInstanceResolutionRoot[];
  resolvableClasses: StudioResolvableClass[];
  maxTargets: number;
  maxCalls: number;
...
export interface RuntimeObservationEnvelope {
  ...
  diagnostics: { errors: number; warnings: number; messageHashes: string[]; truncated: boolean };
}
...
const CAPABILITIES: StudioCapabilityDefinition[] = [
  {
    ...
    description: "Resolve one explicit identity under an allowlisted Studio service root against an allowlisted expected class without reading arbitrary properties.",
...
export const STUDIO_CAPABILITY_SET = createStudioCapabilitySet({
  ...
  policy: {
    allowedRoots: [...STUDIO_INSTANCE_RESOLUTION_ROOTS],
...
export function createStudioCapabilitySet(input: Omit<StudioCapabilitySet, "kind" | "id" | "hash">): StudioCapabilitySet {
  const canonical = {
    ...
    policy: { ...input.policy, allowedRoots: [...input.policy.allowedRoots].sort() as StudioInstanceResolutionRoot[], resolvableClasses: [...input.policy.resolvableClasses].sort() as StudioResolvableClass[] }
  };
  const hash = contentHash(stableJson(canonical));
  const value: StudioCapabilitySet = { kind: "StudioCapabilitySet", id: `studio_capability_set_${hash.slice(0, 24)}`, hash, ...canonical };
...
export function assertStudioCapabilitySet(value: unknown): asserts value is StudioCapabilitySet {
  if (!isRecord(value) || value.kind !== "StudioCapabilitySet" || !isId(value.id) || !isHash(value.hash) || !Array.isArray(value.capabilities) || !isRecord(value.policy)) throw new Error("Invalid StudioCapabilitySet");
  ...
  const canonical = { capabilities: value.capabilities, policy: value.policy };
...
export function createStudioExecutionPlan(input: Omit<StudioExecutionPlan, "kind" | "id" | "hash">): StudioExecutionPlan {
...
export function assertStudioExecutionPlan(value: unknown): asserts value is StudioExecutionPlan {
  ...
  const canonical = canonicalExecutionInput(payload as Omit<StudioExecutionPlan, "kind" | "id" | "hash">);
  const expected = contentHash(stableJson(canonical));
...
export function createRuntimeEvalDefinition(input: Omit<RuntimeEvalDefinition, "kind" | "id" | "hash">): RuntimeEvalDefinition {
...
export function createRuntimeEvaluatorConfiguration(input: Omit<RuntimeEvaluatorConfiguration, "kind" | "id" | "hash">): RuntimeEvaluatorConfiguration {
...
function assertTargetsAndCalls(targets: unknown[], calls: unknown[], budget: StudioExecutionBudget): void {
  ...
  const targetById = new Map(targets.map((target) => [(target as StudioRuntimeTarget).id, target as StudioRuntimeTarget]));
  ...
  for (const call of calls as StudioCapabilityCall[]) {
    ...
    if (call.capability !== "instance.resolve" && target.expectedClass !== "BasePart") throw new Error("BasePart observations require a BasePart runtime target");
    if (call.capability !== "instance.resolve" && !isWorkspacePath(target.path)) throw new Error("BasePart observations are restricted to Workspace targets");
...
function isCapabilityDefinition(value: unknown): value is StudioCapabilityDefinition { return isRecord(value) && ["instance.resolve", "base_part.position", "base_part.position_series"].includes(String(value.name)) && isString(value.description) && isRecord(value.inputSchema) && isRecord(value.outputSchema); }
function isCapabilityPolicy(value: unknown): value is StudioCapabilityPolicy { return isRecord(value) && Array.isArray(value.allowedRoots) && stableJson(value.allowedRoots) === stableJson([...STUDIO_INSTANCE_RESOLUTION_ROOTS]) && Array.isArray(value.resolvableClasses) && stableJson(value.resolvableClasses) === stableJson([...STUDIO_RESOLVABLE_CLASSES].sort()) && isPositiveInteger(value.maxTargets) && isPositiveInteger(value.maxCalls) && isPositiveInteger(value.maxSamplesPerSeries) && isPositiveInteger(value.minSampleIntervalMs) && isPositiveInteger(value.maxSampleIntervalMs) && value.minSampleIntervalMs <= value.maxSampleIntervalMs && isPositiveInteger(value.maxExecutionMs) && isPositiveInteger(value.maxResultBytes); }
function isTarget(value: unknown): value is StudioRuntimeTarget { return isRecord(value) && isId(value.id) && isInstanceResolutionPath(value.path) && STUDIO_RESOLVABLE_CLASSES.includes(value.expectedClass as StudioResolvableClass); }
...
function isRuntimeCapabilityResult(value: unknown): value is RuntimeCapabilityResult { if (!isRecord(value) || !isId(value.id) || !isId(value.targetId)) return false; if (value.capability === "instance.resolve") return ["resolved", "missing", "class_mismatch"].includes(String(value.status)) && (value.path === undefined || isInstanceResolutionPath(value.path)) && (value.className === undefined || isString(value.className)); if (value.capability === "base_part.position") return ["ok", "unavailable"].includes(String(value.status)) && (value.position === undefined || isVector3(value.position)) && (value.elapsedMs === undefined || isNonNegativeFinite(value.elapsedMs)); return value.capability === "base_part.position_series" && ["ok", "unavailable"].includes(String(value.status)) && (value.samples === undefined || (Array.isArray(value.samples) && value.samples.length <= STUDIO_CAPABILITY_SET.policy.maxSamplesPerSeries && value.samples.every((sample, index) => isRecord(sample) && sample.sequence === index + 1 && isNonNegativeFinite(sample.elapsedMs) && isVector3(sample.position)))); }
function isDiagnostics(value: unknown): value is RuntimeObservationEnvelope["diagnostics"] { return isRecord(value) && isNonNegativeInteger(value.errors) && isNonNegativeInteger(value.warnings) && typeof value.truncated === "boolean" && Array.isArray(value.messageHashes) && value.messageHashes.length <= 128 && value.messageHashes.every(isHash); }
...
function isInstanceResolutionPath(value: unknown): value is string {
  if (!isString(value) || value.includes("\\0")) return false;
  const parts = value.split("/");
  return parts.length >= 2 && STUDIO_INSTANCE_RESOLUTION_ROOTS.includes(parts[0] as StudioInstanceResolutionRoot) && parts.every((part) => part.length > 0 && part !== "." && part !== "..");
}
function isWorkspacePath(value: unknown): value is string { return isInstanceResolutionPath(value) && value.startsWith("Workspace/"); }
