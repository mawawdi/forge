...
for (const model of [
  "CreatorMutationTransaction",
  "ExistingProjectIntelligence",
  "RojoSourceAuthority",
  ...
  try {
    await run(
      ...
      [
        ...
        metadir,
        "-config",
        resolve(root, `formal/${model}.cfg`),
        resolve(root, `formal/${model}.tla`),
      ],
      root,
    );
  } finally {
  ...
  process.stdout.write(`TLC completed the ${model} model with one worker.\n`);
}
...
async function assertVerifiedJar(path, expected) {
  const info = await lstat(path).catch((error) => {
    if (isMissing(error))
      throw new Error("Pinned TLA+ JAR is missing. Run npm run formal:setup first.");
    throw error;
  });
  if (info.isSymbolicLink() || !info.isFile())
    throw new Error("Pinned TLA+ JAR is not a regular file");
...
async function assertJava11OrLater() {
  ...
  if (!Number.isInteger(major) || major < 11)
    throw new Error(`Java 11 or newer is required for TLC (found: ${result.output.trim()})`);
}
