...
const validRefreshArguments =
  ...
  args.every(
    (argument, index) =>
      ...
      (index > 0 && args[index - 1] === "--source-root"),
  ) &&
  (!clone || sourceRootArgument === undefined) &&
  (clone || typeof sourceRootArgument === "string");
if (!check && !validRefreshArguments) {
  throw new Error(
    "Usage: node scripts/generate-roblox-api-catalog.mjs --check | --refresh --source-root <creator-docs-checkout> | --refresh --clone",
  );
}
...
if (check) {
  ...
  try {
    const checkoutRoot = clone
      ...
      : await checkedSourceRoot(sourceRootArgument, sourceContract);
...
async function clonePinnedSource(directory, source) {
  ...
  if (stdout.trim() !== source.commit)
    throw new Error(
      `Pinned source commit unavailable from shallow clone: expected ${source.commit}, received ${stdout.trim()}`,
    );
  return checkedSourceRoot(directory, source);
}

async function checkedSourceRoot(value, source) {
  ...
  let checkoutCommit;
  try {
    ({ stdout: checkoutRoot } = await execFileAsync("git", [
      "-C",
      rootPath,
      "rev-parse",
      "--show-toplevel",
    ]));
    ({ stdout: checkoutCommit } = await execFileAsync("git", [
      "-C",
      rootPath,
      "rev-parse",
      "HEAD",
    ...
    throw new Error(`Official API source must be a Git checkout at ${source.commit}`);
  }
  if ((await realpath(checkoutRoot.trim())) !== rootPath)
    throw new Error(`Official API source root must be the checkout root: ${rootPath}`);
  if (checkoutCommit.trim() !== source.commit)
    throw new Error(
      `Official API source commit mismatch: expected ${source.commit}, received ${checkoutCommit.trim()}`,
    );
  return rootPath;
}
...
async function compileCatalog(sourceRoot, source) {
  ...
  if (!engineRoot.startsWith(`${sourceRoot}/`))
    throw new Error("Engine reference path escapes source root");
  ...
  for (const directory of sourceDirectories) {
    ...
    for (const entry of entries) {
      if (entry.isSymbolicLink())
        throw new Error(
          `Official API source must not contain a symlink: ${join(directoryPath, entry.name)}`,
        );
      if (!entry.isFile()) {
        if (entry.isDirectory())
          throw new Error(
            `Official API source must be flat in ${directory}: ${join(directoryPath, entry.name)}`,
          );
        continue;
      }
    ...
    documentsByDirectory.set(
      directory,
      files.sort((left, right) => compareText(left.sourceFile, right.sourceFile)),
    );
  }
  const sourceTreeHash = sourceTreeDigest(sourceFiles);
  if (sourceTreeHash !== source.sourceTreeHash)
    throw new Error(
      `Official API source tree hash drift: expected ${source.sourceTreeHash}, received ${sourceTreeHash}`,
    );

  const classes = (documentsByDirectory.get("classes") ?? [])
    .map(({ sourceFile, sourceFileHash, yaml }) => normalizeClass(yaml, sourceFile, sourceFileHash))
    .sort(byName);
  const datatypes = (documentsByDirectory.get("datatypes") ?? [])
    .map(({ sourceFile, sourceFileHash, yaml }) =>
      normalizeDatatype(yaml, sourceFile, sourceFileHash),
    )
    .sort(byName);
  const enums = (documentsByDirectory.get("enums") ?? [])
    .map(({ sourceFile, sourceFileHash, yaml }) => normalizeEnum(yaml, sourceFile, sourceFileHash))
    .sort(byName);
  const globalMembers = (documentsByDirectory.get("globals") ?? [])
    .flatMap(({ sourceFile, sourceFileHash, yaml }) =>
      normalizeGlobalMembers(yaml, sourceFile, sourceFileHash),
    )
    .sort(byMember);
  const libraries = (documentsByDirectory.get("libraries") ?? [])
    .map(({ sourceFile, sourceFileHash, yaml }) =>
      normalizeLibrary(yaml, sourceFile, sourceFileHash),
    )
    .sort(byName);
...
function normalizeClassMembers(value, kind, className, sourceFile, sourceFileHash) {
  ...
  const normalized = entries.map((entry, index) => {
    ...
    const serialization =
      kind === "property"
        ? normalizeSerialization(member.serialization, `${className}.${name}`)
        : undefined;
    return compact({
      id: "",
      kind,
      ...
      valueType:
        kind === "property"
          ? requiredString(member.type, `property type for ${className}.${name}`)
          : undefined,
      parameters:
        member.parameters === undefined
          ? undefined
          : normalizeParameters(member.parameters, `${className}.${name}`),
      returns:
        member.returns === undefined
          ? undefined
          : normalizeReturns(member.returns, `${className}.${name}`),
...
function normalizeDatatypeMembers(value, kind, datatypeName, sourceFile, sourceFileHash) {
  ...
  const normalized = entries.map((entry, index) => {
    ...
    return compact({
      ...
      valueType: ["constant", "property"].includes(kind)
        ? requiredString(member.type, `${kind} type for ${datatypeName}.${name}`)
        : kind === "math_operation"
          ? requiredString(member.return_type, `math return type for ${datatypeName}.${name}`)
          : undefined,
      operandTypes:
        kind === "math_operation"
          ? [
              requiredString(member.type_a, `math left operand for ${datatypeName}.${name}`),
              requiredString(member.type_b, `math right operand for ${datatypeName}.${name}`),
            ]
          : undefined,
      parameters:
        member.parameters === undefined
          ? undefined
          : normalizeParameters(member.parameters, `${datatypeName}.${name}`),
      returns:
        member.returns === undefined
          ? undefined
          : normalizeReturns(member.returns, `${datatypeName}.${name}`),
  ...
  return withOrdinals(
    normalized,
    (entry) => `datatype_member:${datatypeName}:${kind}:${entry.name}`,
  );
}
...
function normalizeEnum(value, sourceFile, sourceFileHash) {
  ...
  const items = array(document.items, `enum items for ${name}`)
    .map((entry, index) => {
      ...
      if (!Number.isSafeInteger(enumValue))
        throw new Error(`Enum value must be a safe integer: ${name}.${itemName}`);
      return {
...
function normalizeGlobalMemberKind(value, kind, scope, sourceFile, sourceFileHash) {
  ...
  const normalized = entries.map((entry, index) => {
    ...
    return compact({
      ...
      valueType:
        kind === "property"
          ? requiredString(member.type, `global property type for ${name}`)
          : undefined,
      parameters:
        member.parameters === undefined ? undefined : normalizeParameters(member.parameters, name),
...
function normalizeLibraryMemberKind(value, kind, libraryName, sourceFile, sourceFileHash) {
  ...
  const normalized = entries.map((entry, index) => {
    ...
    const sourceName = requiredString(
      member.name,
      `${kind} member name for library ${libraryName}`,
    );
    const prefix = `${libraryName}.`;
    if (!sourceName.startsWith(prefix) || sourceName.length === prefix.length)
      throw new Error(`Invalid library member name: ${sourceName}`);
    ...
    return compact({
      ...
      valueType:
        kind === "property"
          ? requiredString(member.type, `library property type for ${sourceName}`)
          : undefined,
      parameters:
        member.parameters === undefined
          ? undefined
          : normalizeParameters(member.parameters, sourceName),
      returns:
        member.returns === undefined ? undefined : normalizeReturns(member.returns, sourceName),
...
function withOrdinals(entries, baseId) {
  ...
  for (const entry of entries) {
    ...
    if (group === undefined) byBaseId.set(id, [entry]);
  ...
  for (const [id, group] of byBaseId) {
    ...
    for (let index = 0; index < group.length; index += 1)
      result.push({
        ...group[index],
        id: group.length === 1 ? id : `${id}:${index + 1}`,
      });
  }
  return result;
}
...
function normalizeParameters(value, label) {
  return array(value, `parameters for ${label}`).map((entry, index) => {
    ...
    if (
      ...
      throw new Error(`Invalid parameter default for ${label}`);
    return compact({
...
function normalizeSecurity(value, label) {
  ...
  return compact({
    read: optionalString(security.read, `security read for ${label}`),
    write: optionalString(security.write, `security write for ${label}`),
  });
}

function normalizeSerialization(value, label) {
  const serialization = object(value, `serialization for ${label}`);
  if (typeof serialization.can_load !== "boolean" || typeof serialization.can_save !== "boolean")
    throw new Error(`Invalid serialization for ${label}`);
...
function classMemberName(value, className, kind) {
  ...
  if (!value.startsWith(prefix) || value.length === prefix.length)
    throw new Error(`Invalid ${kind} member name: ${value}`);
...
function validateSourceContract(value) {
  ...
  exactKeys(
    source,
    ["kind", "repository", "commit", "engineReferencePath", "sourceTreeHash", "counts"],
    "Roblox API source contract",
  );
  if (
    source.kind !== "RobloxApiCatalogSource" ||
    source.repository !== "https://github.com/Roblox/creator-docs.git" ||
    !isCommit(source.commit) ||
    source.engineReferencePath !== "content/en-us/reference/engine" ||
    !isHash(source.sourceTreeHash)
  )
    throw new Error("Malformed Roblox API source contract");
...
function validateCatalog(value, sourceContract) {
  ...
  if (stableJson(catalog.source) !== stableJson(sourceContract))
    throw new Error("Catalog source pin/count/hash drift from roblox-api-source.json");
  ...
  validateSortedUnique(
    classes.map((entry) =>
      requiredString(object(entry, "catalog class").name, "catalog class name"),
    ),
    "catalog classes",
  );
  validateSortedUnique(
    datatypes.map((entry) =>
      requiredString(object(entry, "catalog datatype").name, "catalog datatype name"),
    ),
    "catalog datatypes",
  );
  validateSortedUnique(
    enums.map((entry) => requiredString(object(entry, "catalog enum").name, "catalog enum name")),
    "catalog enums",
  );
  validateSortedUnique(
    globalMembers.map((entry) =>
      requiredString(object(entry, "catalog global member").id, "catalog global member id"),
    ),
    "catalog global members",
  );
  validateSortedUnique(
    libraries.map((entry) =>
      requiredString(object(entry, "catalog library").name, "catalog library name"),
    ),
    "catalog libraries",
  );
  ...
  const ids = new Set();
  for (const entry of classes)
    validateCatalogClass(object(entry, "catalog class"), classNames, ids);
  ...
  for (const entry of globalMembers)
    validateCatalogGlobalMember(object(entry, "catalog global member"), ids);
  for (const entry of libraries) validateCatalogLibrary(object(entry, "catalog library"), ids);
  const actualCounts = countCatalog({
    classes,
    datatypes,
    enums,
    globalMembers,
  ...
  if (
    stableJson(actualCounts) !== stableJson(catalog.counts) ||
    stableJson(actualCounts) !== stableJson(sourceContract.counts)
  )
    throw new Error("Catalog member counts drift from source contract");
  ...
  if (!isHash(catalog.contentHash) || sha256(stableJson(material)) !== catalog.contentHash)
    throw new Error("Catalog content hash does not match its normalized content");
}
...
function validateCatalogClass(entry, classNames, ids) {
  exactKeys(
    ...
    [
      "id",
      "name",
      ...(entry.superclass === undefined ? [] : ["superclass"]),
      "tags",
      "deprecated",
      "members",
      "sourceFile",
  ...
  if (
    entry.id !== `class:${entry.name}` ||
    (entry.superclass !== undefined &&
      (!isNonEmptyString(entry.superclass) || !classNames.has(entry.superclass))) ||
    typeof entry.deprecated !== "boolean"
  )
    throw new Error(`Malformed catalog class: ${String(entry.name)}`);
  validateTags(entry.tags, `catalog class tags for ${entry.name}`);
  validateSourceFile(entry.sourceFile, entry.sourceFileHash, `catalog class ${entry.name}`);
  if (entry.sourceFile !== `classes/${entry.name}.yaml`)
    throw new Error(`Catalog class source path mismatch: ${entry.name}`);
  addId(ids, entry.id);
  const members = array(entry.members, `members for ${entry.name}`);
  validateSortedUnique(
    members.map((member) =>
      requiredString(object(member, "catalog class member").id, "catalog class member id"),
    ),
    `members for ${entry.name}`,
  );
  for (const member of members)
    validateCatalogClassMember(object(member, "catalog class member"), entry.name, ids);
}

function validateCatalogClassMember(entry, className, ids) {
  const optional = [
    entry.valueType === undefined ? [] : ["valueType"],
    entry.parameters === undefined ? [] : ["parameters"],
    entry.returns === undefined ? [] : ["returns"],
    entry.security === undefined ? [] : ["security"],
    entry.serialization === undefined ? [] : ["serialization"],
    entry.threadSafety === undefined ? [] : ["threadSafety"],
  ].flat();
  exactKeys(
    entry,
    [
    ...
    "catalog class member",
  );
  if (
    !isClassMemberKind(entry.kind) ||
    !isNonEmptyString(entry.name) ||
    entry.declaringClass !== className ||
    typeof entry.deprecated !== "boolean"
  )
    throw new Error(`Malformed class member for ${className}`);
  ...
  if (
    entry.kind === "property"
      ? !isNonEmptyString(entry.valueType) || entry.serialization === undefined
      : entry.valueType !== undefined || entry.serialization !== undefined
  )
    throw new Error(`Malformed class member payload: ${entry.id}`);
  validateTags(entry.tags, `catalog class member tags for ${entry.id}`);
  validateTags(entry.capabilities, `catalog class member capabilities for ${entry.id}`);
  validateOptionalMembers(entry, `catalog class member ${entry.id}`);
  validateSourceFile(entry.sourceFile, entry.sourceFileHash, `catalog class member ${entry.id}`);
  addId(ids, entry.id);
}
...
function validateCatalogDatatype(entry, ids) {
  exactKeys(
    entry,
    ["id", "name", "tags", "deprecated", "members", "sourceFile", "sourceFileHash"],
    "catalog datatype",
  );
  if (
    entry.id !== `datatype:${entry.name}` ||
    !isNonEmptyString(entry.name) ||
    typeof entry.deprecated !== "boolean"
  )
    throw new Error(`Malformed catalog datatype: ${String(entry.name)}`);
  validateTags(entry.tags, `catalog datatype tags for ${entry.name}`);
  validateSourceFile(entry.sourceFile, entry.sourceFileHash, `catalog datatype ${entry.name}`);
  if (entry.sourceFile !== `datatypes/${entry.name}.yaml`)
    throw new Error(`Catalog datatype source path mismatch: ${entry.name}`);
  addId(ids, entry.id);
  const members = array(entry.members, `members for ${entry.name}`);
  validateSortedUnique(
    members.map((member) =>
      requiredString(object(member, "catalog datatype member").id, "catalog datatype member id"),
    ),
    `members for ${entry.name}`,
  );
  for (const member of members)
    validateCatalogDatatypeMember(object(member, "catalog datatype member"), entry.name, ids);
}

function validateCatalogDatatypeMember(entry, datatypeName, ids) {
  const optional = [
    entry.valueType === undefined ? [] : ["valueType"],
    entry.operandTypes === undefined ? [] : ["operandTypes"],
    entry.parameters === undefined ? [] : ["parameters"],
    entry.returns === undefined ? [] : ["returns"],
  ].flat();
  exactKeys(
    ...
    "catalog datatype member",
  );
  if (
    !isDatatypeMemberKind(entry.kind) ||
    !isNonEmptyString(entry.name) ||
    entry.declaringDatatype !== datatypeName ||
    typeof entry.deprecated !== "boolean"
  )
    throw new Error(`Malformed datatype member for ${datatypeName}`);
  const base = `datatype_member:${datatypeName}:${entry.kind}:${entry.name}`;
  if (!matchesMemberId(entry.id, base))
    throw new Error(`Malformed datatype member id: ${entry.id}`);
  const needsValueType =
    entry.kind === "constant" || entry.kind === "property" || entry.kind === "math_operation";
  if (
    needsValueType !== isNonEmptyString(entry.valueType) ||
    (entry.kind === "math_operation") !== Array.isArray(entry.operandTypes)
  )
    throw new Error(`Malformed datatype member payload: ${entry.id}`);
  if (
    entry.operandTypes !== undefined &&
    (entry.operandTypes.length !== 2 || entry.operandTypes.some((type) => !isNonEmptyString(type)))
  )
    throw new Error(`Malformed datatype operands: ${entry.id}`);
  validateTags(entry.tags, `catalog datatype member tags for ${entry.id}`);
  validateOptionalMembers(entry, `catalog datatype member ${entry.id}`);
  validateSourceFile(entry.sourceFile, entry.sourceFileHash, `catalog datatype member ${entry.id}`);
  addId(ids, entry.id);
}
...
function validateCatalogEnum(entry, ids) {
  exactKeys(
    entry,
    ["id", "name", "tags", "deprecated", "items", "sourceFile", "sourceFileHash"],
    "catalog enum",
  );
  if (
    entry.id !== `enum:${entry.name}` ||
    !isNonEmptyString(entry.name) ||
    typeof entry.deprecated !== "boolean"
  )
    throw new Error(`Malformed catalog enum: ${String(entry.name)}`);
  validateTags(entry.tags, `catalog enum tags for ${entry.name}`);
  validateSourceFile(entry.sourceFile, entry.sourceFileHash, `catalog enum ${entry.name}`);
  if (entry.sourceFile !== `enums/${entry.name}.yaml`)
    throw new Error(`Catalog enum source path mismatch: ${entry.name}`);
  addId(ids, entry.id);
  const items = array(entry.items, `items for ${entry.name}`);
  validateSortedUnique(
    items.map((item) =>
      requiredString(object(item, "catalog enum item").name, "catalog enum item name"),
    ),
    `items for ${entry.name}`,
  );
  for (const item of items) {
    const parsed = object(item, "catalog enum item");
    exactKeys(parsed, ["id", "name", "value", "tags", "deprecated"], "catalog enum item");
    if (
      parsed.id !== `enum_item:${entry.name}:${parsed.name}` ||
      !isNonEmptyString(parsed.name) ||
      !Number.isSafeInteger(parsed.value) ||
      typeof parsed.deprecated !== "boolean"
    )
      throw new Error(`Malformed catalog enum item: ${entry.name}`);
    validateTags(parsed.tags, `catalog enum item tags for ${parsed.id}`);
    addId(ids, parsed.id);
  }
}
...
function validateCatalogGlobalMember(entry, ids) {
  ...
  exactKeys(
    ...
    "catalog global member",
  );
  if (
    !isGlobalMemberKind(entry.kind) ||
    !isNonEmptyString(entry.name) ||
    !isNonEmptyString(entry.declaringScope) ||
    typeof entry.deprecated !== "boolean"
  )
    throw new Error(`Malformed global member: ${String(entry.name)}`);
  ...
  if ((entry.kind === "property") !== isNonEmptyString(entry.valueType))
    throw new Error(`Malformed global member payload: ${entry.id}`);
  validateTags(entry.tags, `catalog global member tags for ${entry.id}`);
  validateOptionalMembers(entry, `catalog global member ${entry.id}`);
  validateSourceFile(entry.sourceFile, entry.sourceFileHash, `catalog global member ${entry.id}`);
  addId(ids, entry.id);
}
...
function validateCatalogLibrary(entry, ids) {
  ...
  if (
    entry.id !== `library:${entry.name}` ||
    !isNonEmptyString(entry.name) ||
    typeof entry.deprecated !== "boolean"
  )
    throw new Error(`Malformed catalog library: ${String(entry.name)}`);
  validateTags(entry.tags, `catalog library tags for ${entry.name}`);
  validateSourceFile(entry.sourceFile, entry.sourceFileHash, `catalog library ${entry.name}`);
  if (entry.sourceFile !== `libraries/${entry.name}.yaml`)
    throw new Error(`Catalog library source path mismatch: ${entry.name}`);
  addId(ids, entry.id);
  const members = array(entry.members, `members for library ${entry.name}`);
  validateSortedUnique(
    members.map((member) =>
      requiredString(object(member, "catalog library member").id, "catalog library member id"),
    ),
    `members for library ${entry.name}`,
  );
  for (const member of members)
    validateCatalogLibraryMember(object(member, "catalog library member"), entry.name, ids);
}
...
function validateCatalogLibraryMember(entry, libraryName, ids) {
  ...
  if (
    !isLibraryMemberKind(entry.kind) ||
    !isNonEmptyString(entry.name) ||
    entry.declaringLibrary !== libraryName ||
    typeof entry.deprecated !== "boolean"
  )
    throw new Error(`Malformed library member for ${libraryName}`);
  ...
  if ((entry.kind === "property") !== isNonEmptyString(entry.valueType))
    throw new Error(`Malformed library member payload: ${entry.id}`);
  validateTags(entry.tags, `catalog library member tags for ${entry.id}`);
  validateOptionalMembers(entry, `catalog library member ${entry.id}`);
  validateSourceFile(entry.sourceFile, entry.sourceFileHash, `catalog library member ${entry.id}`);
  addId(ids, entry.id);
}

function validateOptionalMembers(entry, label) {
  if (entry.parameters !== undefined)
    for (const parameter of array(entry.parameters, `${label} parameters`)) {
      const parsed = object(parameter, `${label} parameter`);
      exactKeys(
        parsed,
        ["name", "type", ...(parsed.default === undefined ? [] : ["default"])],
        `${label} parameter`,
      );
      if (
        !isNonEmptyString(parsed.name) ||
        ...
        (parsed.default !== undefined &&
          !["string", "number", "boolean"].includes(typeof parsed.default))
      )
        throw new Error(`Malformed ${label} parameter`);
    }
  if (entry.returns !== undefined)
    for (const result of array(entry.returns, `${label} returns`)) {
      const parsed = object(result, `${label} return`);
      exactKeys(parsed, ["type"], `${label} return`);
      if (!isNonEmptyString(parsed.type)) throw new Error(`Malformed ${label} return`);
    }
  if (entry.security !== undefined) {
    const security = object(entry.security, `${label} security`);
    exactKeys(
      security,
      [
        ...(security.read === undefined ? [] : ["read"]),
        ...(security.write === undefined ? [] : ["write"]),
      ],
      `${label} security`,
    );
    if (!isNonEmptyString(security.read) && !isNonEmptyString(security.write))
      throw new Error(`Malformed ${label} security`);
  }
  if (entry.serialization !== undefined) {
    const serialization = object(entry.serialization, `${label} serialization`);
    exactKeys(serialization, ["canLoad", "canSave"], `${label} serialization`);
    if (typeof serialization.canLoad !== "boolean" || typeof serialization.canSave !== "boolean")
      throw new Error(`Malformed ${label} serialization`);
  }
  if (entry.threadSafety !== undefined && !isNonEmptyString(entry.threadSafety))
    throw new Error(`Malformed ${label} thread safety`);
}
...
function validateCounts(value, label) {
  const counts = object(value, `${label} counts`);
  exactKeys(counts, countKeys, `${label} counts`);
  for (const key of countKeys)
    if (!Number.isSafeInteger(counts[key]) || counts[key] < 0)
      throw new Error(`Invalid ${label} count: ${key}`);
}

function sourceTreeDigest(files) {
  const hash = createHash("sha256");
  for (const { sourceFile, bytes } of [...files].sort((left, right) =>
...
async function assertExactOutput(path, expected) {
  ...
  try {
    ...
    if (error?.code === "ENOENT") {
      throw new Error(`Missing generated Roblox API catalog output: ${path}`, {
        cause: error,
      });
    }
    throw error;
  ...
  if (actual !== expected)
    throw new Error(
      `Stale generated Roblox API catalog output: ${path}. Run npm run roblox-api-catalog:refresh`,
    );
}
...
function parseYaml(bytes, path) {
  ...
  if (document.errors.length > 0)
    throw new Error(
      `Malformed YAML ${path}: ${document.errors.map((error) => error.message).join("; ")}`,
    );
...
function normalizedTags(value, label) {
  const tags = stringArray(value, label).sort(compareText);
  validateSortedUnique(tags, label);
  return tags;
}
function deprecated(value) {
  const document = object(value, "deprecated source");
  return (
    normalizedTags(document.tags, "deprecated tags").includes("Deprecated") ||
    (typeof document.deprecation_message === "string" &&
      document.deprecation_message.trim().length > 0)
  );
}
...
function stringArray(value, label) {
  return array(value, label).map((entry) => requiredString(entry, label));
}
...
function validateTags(value, label) {
  validateSortedUnique(stringArray(value, label), label);
}
function validateSourceFile(sourceFile, sourceFileHash, label) {
  if (
    !isNonEmptyString(sourceFile) ||
    !sourceDirectories.some((directory) => sourceFile.startsWith(`${directory}/`)) ||
    sourceFile.includes("..") ||
    !isHash(sourceFileHash)
  )
    throw new Error(`Malformed source provenance for ${label}`);
}
function validateSortedUnique(values, label) {
  for (let index = 0; index < values.length; index += 1)
    if (
      !isNonEmptyString(values[index]) ||
      (index > 0 && compareText(values[index - 1], values[index]) >= 0)
    )
...
function addId(ids, id) {
  if (!isNonEmptyString(id) || ids.has(id))
    throw new Error(`Duplicate or malformed catalog id: ${String(id)}`);
  ids.add(id);
}
function matchesMemberId(value, base) {
  return value === base || new RegExp(`^${escapeRegExp(base)}:[1-9][0-9]*$`).test(value);
}
function escapeRegExp(value) {
  return value.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
}
function isClassMemberKind(value) {
  return value === "property" || value === "method" || value === "event" || value === "callback";
}
function isDatatypeMemberKind(value) {
  return (
    value === "constant" ||
    value === "constructor" ||
    value === "function" ||
...
function isGlobalMemberKind(value) {
  return value === "property" || value === "function";
}
function isLibraryMemberKind(value) {
  return value === "property" || value === "function";
}
...
function taggedPrefix(tag, payloadLength) {
  return `${Buffer.byteLength(tag, "utf8")}:${tag}${payloadLength}:`;
}
...
function memberSignature(value) {
  return stableJson({
    name: value.name,
    valueType: value.valueType,
    operandTypes: value.operandTypes,
