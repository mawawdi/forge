...
import { ImmutableJsonArtifactStore, assertArtifactReference, isSafeArtifactLocator, type ArtifactReference } from "../packages/artifact-store/src/index.js";
...
test("writes canonical private content-addressed JSON and deduplicates exact bytes", async () => {
  await withStore(async (root, store) => {
    ...
    assert.equal(first.artifactHash, createHash("sha256").update(`${stableJson({ a: "first", nested: { a: 1, z: true }, z: "last" })}\n`).digest("hex"));
    assert.equal(first.bytes, Buffer.byteLength(`${stableJson({ a: "first", nested: { a: 1, z: true }, z: "last" })}\n`, "utf8"));
    assert.equal((await lstat(join(root, first.locator))).mode & 0o777, 0o600);
    assert.equal(await readFile(join(root, first.locator), "utf8"), `${stableJson({ a: "first", nested: { a: 1, z: true }, z: "last" })}\n`);
...
test("fails closed for missing, tampered, noncanonical, and oversized artifacts", async () => {
  await withStore(async (root, store) => {
    ...
    await writeFile(join(root, noncanonicalReference.locator), noncanonical, { encoding: "utf8", mode: 0o600 });
...
test("rejects unsafe locators and all symlink path components", async () => {
  await withStore(async (root, store) => {
    const reference = await store.write({ safe: true });
    for (const locator of ["../outside.json", "/absolute.json", "C:\\drive.json", "artifacts\\backslash.json", "artifacts//double.json", "./artifact.json"]) {
...
test("never overwrites a content-addressed conflict and supports typed read assertions", async () => {
  await withStore(async (root, store) => {
    ...
    const reference = referenceFor(`artifacts/${createHash("sha256").update(serialized).digest("hex")}.json`, serialized);
    await mkdir(join(root, "artifacts"), { mode: 0o700 });
    await writeFile(join(root, "artifacts", `${reference.artifactHash}.json`), "{\"conflict\":true}\n", { encoding: "utf8", mode: 0o600 });
    ...
    const read = await store.read(valid, (value): asserts value is { answer: number; type: string } => {
...
async function withStore(run: (root: string, store: ImmutableJsonArtifactStore) => Promise<void>): Promise<void> {
...
async function copyFileTree(source: string, destination: string, reference: ArtifactReference): Promise<void> {
