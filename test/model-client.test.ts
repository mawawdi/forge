...
import { CANONICAL_OPENROUTER_MODEL, MODEL_CONTINUATION_MAX_BYTES, OpenRouterModelClient, type ModelTurnRequest } from "../packages/model-client/src/index.js";
...
const REQUEST: ModelTurnRequest = {
  ...
  tools: [
    { name: "project.read", description: "Read one source file.", parameters: { type: "object", properties: { path: { type: "string" } }, required: ["path"], additionalProperties: false } },
    { name: "project.list", description: "List bounded source files only.", parameters: { type: "object", properties: {}, additionalProperties: false } }
  ],
  maxOutputTokens: 512,
  timeoutMs: 1_000
};

const REASONING_DETAIL = { type: "reasoning.encrypted", data: "opaque-reasoning-block", id: "reasoning-1", format: "openai-responses-v1" };
...
test("AI SDK Core sends one locked-down OpenRouter step and replays opaque reasoning continuation", async () => {
  ...
  const fetchImpl: typeof fetch = async (_input, init) => {
    ...
    if (attempt === 1) return response({ content: null, reasoning_details: [REASONING_DETAIL], tool_calls: [{ id: "call-1", type: "function", function: { name: "project_list", arguments: "{}" } }] }, "tool_calls", "response-1");
    return response({ content: "Inspection complete.", reasoning_details: [] }, "stop", "response-2");
  };
  ...
  assert.deepEqual(first.message.toolCalls, [{ id: "call-1", name: "project.list", arguments: {} }]);
  ...
  const second = await client.complete({ ...REQUEST, messages: [...REQUEST.messages, first.message, { role: "tool", toolCallId: "call-1", name: "project.list", content: "{\"ok\":true}" }] });
  ...
  assert.deepEqual(firstBody.provider, { only: ["openai"], allow_fallbacks: false, require_parameters: true });
  ...
  assert.deepEqual(sentTools.map((entry) => entry.function.name), ["project_read", "project_list"]);
...
test("AI SDK Core performs one HTTP attempt and normalizes bounded provider errors", async () => {
  ...
  const fetchImpl: typeof fetch = async () => {
    attempts += 1;
    return new Response(JSON.stringify({ error: { message: "sensitive provider pipeline details" } }), { status: 429, headers: { "content-type": "application/json" } });
  };
...
test("malformed or oversized continuation fails before transport and remains bounded", async () => {
  let attempts = 0;
  const client = new OpenRouterModelClient({ apiKey: "secret", fetchImpl: async () => { attempts += 1; throw new Error("must not execute"); } });
  const payload = [{ role: "assistant", content: "x".repeat(MODEL_CONTINUATION_MAX_BYTES) }];
  const result = await client.complete({
    ...REQUEST,
    messages: [{ role: "assistant", content: "", toolCalls: [], continuation: { transport: "openrouter-ai-sdk-core", payload, hash: "0".repeat(64), bytes: MODEL_CONTINUATION_MAX_BYTES + 1 } }]
  });
  assert.equal(result.kind, "provider_error");
...
test("timeouts and malformed HTTP envelopes remain pre-trial provider failures", async () => {
  const timeoutFetch: typeof fetch = async (_input, init) => new Promise((_resolve, reject) => {
    const fallback = setTimeout(() => reject(new Error("AI SDK did not abort the bounded request")), 250);
    const abort = () => { clearTimeout(fallback); reject(new DOMException("The operation was aborted", "AbortError")); };
    if (init?.signal?.aborted) abort(); else init?.signal?.addEventListener("abort", abort, { once: true });
  });
  const timeout = await new OpenRouterModelClient({ apiKey: "secret", fetchImpl: timeoutFetch }).complete({ ...REQUEST, timeoutMs: 10 });
  assert.equal(timeout.kind, "provider_error");
  if (timeout.kind === "provider_error") { assert.equal(timeout.errorClass, "timeout"); assert.equal(timeout.responseFacts.responseId, null); }

  const malformed = await new OpenRouterModelClient({ apiKey: "secret", fetchImpl: async () => new Response("{\"unexpected\":true}", { status: 200, headers: { "content-type": "application/json" } }) }).complete(REQUEST);
...
test("AI SDK parses envelopes while Forge retains semantic tool-argument authority", async () => {
  const semanticallyInvalid = await new OpenRouterModelClient({ apiKey: "secret", fetchImpl: async () => response({ content: null, tool_calls: [{ id: "bad-semantic-input", type: "function", function: { name: "project_read", arguments: "{}" } }] }, "tool_calls", "response-semantic") }).complete(REQUEST);
  assert.equal(semanticallyInvalid.kind, "assistant");
  if (semanticallyInvalid.kind === "assistant") assert.deepEqual(semanticallyInvalid.message.toolCalls[0]?.arguments, {});

  const malformedArguments = await new OpenRouterModelClient({ apiKey: "secret", fetchImpl: async () => response({ content: null, tool_calls: [{ id: "bad-json", type: "function", function: { name: "project_read", arguments: "not-json" } }] }, "tool_calls", "response-invalid") }).complete(REQUEST);
  assert.equal(malformedArguments.kind, "assistant");
  if (malformedArguments.kind === "assistant") assert.equal(malformedArguments.message.toolCalls[0]?.arguments, "not-json");
});
