...
const doorIdentity = { kind: "forge_attribute" as const, stableId: "runtime-door" };
const doorEvidenceTarget = {
...
function plan() {
  return createStudioExecutionPlan({
    ...
    targets: [
      { id: "door", identity: doorIdentity, path: "Workspace/Door", expectedClass: "BasePart" },
    ],
    calls: [
  ...
  return [
    {
      ...
      key: studioEvidenceFactKey("runtime_resolution", doorEvidenceTarget, "resolve-door"),
      target: doorEvidenceTarget,
      callId: "resolve-door",
      runtimeTargetId: "door",
      capability: "instance.resolve",
      result: { status: "observed", value: { path: "Workspace/Door", className: "Part" } },
    },
    {
      kind: "position_series",
      key: studioEvidenceFactKey("position_series", doorEvidenceTarget, "sample-door"),
      target: doorEvidenceTarget,
      callId: "sample-door",
      runtimeTargetId: "door",
      capability: "base_part.position_series",
...
test("runtime plans compile the manifest-bound evidence projection", () => {
  ...
  assert.deepEqual(
    executionPlan.evidenceProjection.requirements.map((requirement) => requirement.kind),
    ["position_series", "runtime_resolution"],
  );
});
...
test("fixed runtime property observations stay inside the generated manifest", () => {
  const executionPlan = createStudioExecutionPlan({
    purpose: "runtime_evaluation",
    binding: {
      runId: "runtime-property-run",
      correlationId: "runtime-property-correlation",
      sessionId: "runtime-property-session",
      projectId: "runtime-property-project",
      project,
      projectRevisionHash: hash,
    },
    targets: [
      {
        id: "label",
        identity: { kind: "forge_attribute", stableId: "runtime-label" },
        path: "StarterGui/Hud/Label",
        expectedClass: "TextLabel",
      },
    ],
    calls: [
      ...
      {
        id: "read-label-text",
        capability: "instance.property",
        targetId: "label",
        propertyName: "Text",
      },
      {
        id: "sample-label-transparency",
  ...
  assert.deepEqual(
    executionPlan.calls.map((call) => call.capability),
    ["instance.resolve", "instance.property", "instance.property_series"],
  );
  assert.deepEqual(
    executionPlan.evidenceProjection.requirements.map((requirement) => [
      requirement.kind,
      requirement.propertyName,
    ]),
    [
      ["runtime_property", "Text"],
      ["runtime_property_series", "TextTransparency"],
      ["runtime_resolution", undefined],
    ],
  );
  assert.throws(
    () =>
      createStudioExecutionPlan({
        ...
        binding: {
          ...
          runId: "runtime-property-invalid",
          correlationId: "runtime-property-invalid-correlation",
        },
        calls: [
          { id: "resolve-label", capability: "instance.resolve", targetId: "label" },
          {
...
test("runtime plans canonicalize dependency order before validating calls", () => {
  const executionPlan = createStudioExecutionPlan({
    ...
    targets: [
      {
        id: "target-b",
        identity: { kind: "forge_attribute", stableId: "runtime-target-b" },
        path: "Workspace/DoorB",
        expectedClass: "BasePart",
      },
      {
        id: "target-a",
        identity: { kind: "forge_attribute", stableId: "runtime-target-a" },
        path: "Workspace/DoorA",
        expectedClass: "BasePart",
      },
    ],
    calls: [
...
test("independent runtime series share one concurrent execution window", () => {
  assert.doesNotThrow(() =>
    createStudioExecutionPlan({
      purpose: "creator_verification",
      binding: {
        runId: "concurrent-series-run",
        correlationId: "concurrent-series-correlation",
        sessionId: "concurrent-series-session",
        projectId: "concurrent-series-project",
        project,
        projectRevisionHash: hash,
      },
      targets: [
        {
          id: "door-a",
          identity: { kind: "forge_attribute", stableId: "runtime-door-a" },
          path: "Workspace/DoorA",
          expectedClass: "BasePart",
        },
        {
          id: "door-b",
          identity: { kind: "forge_attribute", stableId: "runtime-door-b" },
          path: "Workspace/DoorB",
          expectedClass: "BasePart",
        },
      ],
      calls: [
        { id: "resolve-a", capability: "instance.resolve", targetId: "door-a" },
        { id: "resolve-b", capability: "instance.resolve", targetId: "door-b" },
        {
          id: "series-a",
          capability: "base_part.position_series",
          targetId: "door-a",
      ...
      budget: { maxExecutionMs: 20_000, maxResultBytes: 8_192 },
      observationWindowMs: 15_000,
    }),
  );
});
...
test("runtime grading consumes complete runtime evidence rather than a snapshot envelope", () => {
  ...
  const definition = createRuntimeEvalDefinition({
    requirementSetId: "runtime-requirements",
    evaluatorViewId: "runtime-view",
    evaluatorViewHash: hash,
    acceptanceSpecId: "runtime-acceptance",
    ...
    assertions: [
      ...
      },
      {
        ...
        observationId: "sample-door",
        quantizationStuds: 1,
        minimumDistinctPositions: 3,
      },
    ],
  });
  assert.deepEqual(
...
test("unavailable runtime facts cannot be promoted to a matched projection", () => {
  ...
  const unavailable = createStudioEvidenceEnvelope({
    ...
    facts: observedRuntimeFacts().map((fact) =>
      fact.kind === "position_series"
        ? { ...fact, result: { status: "unavailable" as const, code: "play_solo_unavailable" } }
        : fact,
    ),
  });
  ...
  assert.deepEqual(
    runtimeResultsFromEvidence(unavailable, executionPlan.evidenceProjection).find(
      (result) => result.id === "sample-door",
    ),
    {
      id: "sample-door",
      capability: "base_part.position_series",
