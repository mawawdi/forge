...
test("candidate patch targets retain observed Script, LocalScript, and extensionless Rojo ModuleScript classes", () => {
  const serverSource = "return true";
  const clientSource = "return false";
  const moduleSource = "return {}";
  const patchSet = { kind: "PatchSet", schemaVersion: 1, id: "candidate", projectHash: "seed", mechanicContractId: "contract", operations: [{ type: "replace_text", path: "src/server/CollectFruit.server.luau", beforeHash: createHash("sha256").update(serverSource).digest("hex"), before: serverSource, after: serverSource }, { type: "replace_text", path: "src/client/CollectFruitClient.client.luau", beforeHash: createHash("sha256").update(clientSource).digest("hex"), before: clientSource, after: clientSource }, { type: "replace_text", path: "src/ReplicatedStorage/ClientActions/CollectFruitAction.luau", beforeHash: createHash("sha256").update(moduleSource).digest("hex"), before: moduleSource, after: moduleSource }], expectedEffects: [], provenance: { generatedAt: time }, bounds: { maxFiles: 3, maxAddedLines: 0, maxRemovedLines: 0 } } satisfies PatchSet;
  const observation = { kind: "StudioSnapshotObservation", schemaVersion: 3, project: { name: "Fixture", placeId: 0, universeId: 0 }, capturedAt: time, instances: [{ stableId: "server", path: "ServerScriptService/CollectFruit.server.luau", className: "Script", properties: [], attributes: [], tags: [] }, { stableId: "client", path: "StarterPlayer/StarterPlayerScripts/CollectFruitClient.client.luau", className: "LocalScript", properties: [], attributes: [], tags: [] }, { stableId: "module", path: "ReplicatedStorage/ClientActions/CollectFruitAction", className: "ModuleScript", properties: [], attributes: [], tags: [] }], scripts: [{ stableId: "server", path: "ServerScriptService/CollectFruit.server.luau", executionContext: "server", sourceHash: createHash("sha256").update(serverSource).digest("hex"), source: serverSource }, { stableId: "client", path: "StarterPlayer/StarterPlayerScripts/CollectFruitClient.client.luau", executionContext: "client", sourceHash: createHash("sha256").update(clientSource).digest("hex"), source: clientSource }, { stableId: "module", path: "ReplicatedStorage/ClientActions/CollectFruitAction", executionContext: "shared", sourceHash: createHash("sha256").update(moduleSource).digest("hex"), source: moduleSource }], remotes: [] } satisfies StudioSnapshotObservation;
  const targets = resolveStudioPatchTargets(patchSet, observation);
  assert.deepEqual(targets.map((entry) => entry.target?.className), ["Script", "LocalScript", "ModuleScript"]);
});
...
test("atomic server result creates an authoritative verified StudioProof", async () => {
  ...
  capture.accept(accepted(plan, snapshot)); capture.accept(lifecycle("PlaytestStarted", plan, snapshot)); capture.accept({ kind: "StudioProtocolMessage", schemaVersion: 10, direction: "plugin_to_backend", type: "StudioTestResult", messageId: "result", sentAt: time, payload: envelope(plan, snapshot) }); capture.accept(lifecycle("PlaytestStopped", plan, snapshot));
...
test("atomic result rejects duplicates and wrong nonces; Output is not a protocol message", async () => {
  const { contract, snapshot } = await fixture(); const plan = collectFruitTestPlan(contract, snapshot); const result = envelope(plan, snapshot); const message = { kind: "StudioProtocolMessage" as const, schemaVersion: 10 as const, direction: "plugin_to_backend" as const, type: "StudioTestResult" as const, messageId: "result", sentAt: time, payload: result };
...
test("the plugin arms first and uses the canary-proven Play Solo result contract", async () => {
  ...
  assert.match(harness, /EXPECTED_ASSERTION_IDS/); assert.match(harness, /supportsAssertions/); assert.match(harness, /supportsResults/); assert.match(runtime, /proof:supportsPlan\(payload\)/); assert.match(proof, /run\.harness\.harness\.supportsResults\(envelope\.assertions\)/);
