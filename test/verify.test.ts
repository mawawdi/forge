...
test("CLI help exposes prompt-only creator sessions and registered experiments with no loose creator build", () => {
  ...
  assert.match(result.stdout, /Forge commands/); assert.match(result.stdout, /forge creator serve/); assert.match(result.stdout, /forge creator start/); assert.match(result.stdout, /forge creator replay-verification/); assert.match(result.stdout, /forge creator approve-plan/); assert.match(result.stdout, /forge creator approve-changes/); assert.match(result.stdout, /forge creator start-checks/); assert.match(result.stdout, /forge creator accept/); assert.match(result.stdout, /forge experiment register/); assert.match(result.stdout, /--runtime-configuration/); assert.match(result.stdout, /forge experiment build/); assert.match(result.stdout, /forge experiment evaluate/); assert.match(result.stdout, /forge studio canary/); assert.match(result.stdout, /forge studio bridge/); assert.match(result.stdout, /forge verify/); assert.match(result.stdout, /forge trace show/);
